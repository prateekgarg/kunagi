...
import ilarkesto.mda.generator.ActivityCatalogGenerator;
...
public class KunagiModelApplication extends AGeneratorApplication {
	...
	public EntityModel getProjectModel() {
		if (projectModel == null) {
			...
			projectModel.addStringProperty("productLabel").setTooltip(
					"This is the name of the product, which is created within this project.");
			projectModel.addStringProperty("shortDescription").setTooltip(
			...
			projectModel.addStringProperty("longDescription").setRichtext(true)
			...
			projectModel
			.addStringProperty("externalTrackerUrlTemplate")
			.setTooltip(
					"URL templat to open an issue or story in the external tracker, using the associated ID. Example: http://localhost/mytracker?issueId=${id}");
	...
	public EntityModel getRequirementModel() {
		if (requirementModel == null) {
			...
			requirementModel.addStringProperty("externalTrackerId").setEditablePredicate("editable")
					.setLabel("External ID").setTooltip("External system ID. From other bug tracker or else.");
	...
	public EntityModel getTaskModel() {
		if (taskModel == null) {
			...
			taskModel.addSetReference("impediments", getImpedimentModel()).setEditablePredicate("editable")
					.setTooltip("Impediments which cause problems.");
	...
	protected void onGeneration() {
		...
		new ActivityCatalogGenerator(ScrumGwtApplication.class.getPackage().getName()).generate();
